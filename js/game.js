    ...
    if (!options.isGameOver) {
        ...
        if (fps.elapsed >= fps.interval) {
            ...
            if (color.flickering) {
                ...
                ctx.shadowColor =
                    color.getAlphaString(color.value,
                    color.glowingCoefficient);
            ...
            fps.current = Math.round(
                1000 / (fps.sinceStart / ++fps.frameCount) * 100) / 100;
...
function start() {
    ...
    for (var i = 0; i < stars.canvas.width; i += starsIntensity) {
        for (var j = 0; j < stars.canvas.height; j += starsIntensity) {
            ...
            stars.fillStyle =
                color.getString([starsColor,starsColor,starsColor]);

            if (getRandomInt(0,2))
                stars.fillRect(i + getRandomInt(0,starsIntensity),
    ...
    setInterval(function() {
        document.getElementById("fps").innerHTML =
            Math.round(1000 / fps.elapsed) + " fps.";
    }, 100);
}
...
spaceShip = {
    ...
    update: function() {
        ...
        ctx.fillRect(this.x + options.cellSize, this.y + options.cellSize,
            options.cellSize * 2, options.cellSize); 
        ctx.fillRect(this.x, this.y + options.cellSize * 2,
            options.cellSize * 2, options.cellSize);
    }
},
...
color = {
    ...
    change: function(from) {
        ...
        ctx.shadowColor = this.getAlphaString(this.value,
            this.glowingCoefficient);
...
shot = {
    ...
    make: function() {
        this.positionRightNow = this.coordConstX +
            options.cellSize * 3 + this.coord;

        ctx.fillRect(this.positionRightNow,
            this.coordConstY + options.cellSize,
            options.cellSize, options.cellSize);
        
        this.coord += options.cellSize * 5;

        if (this.positionRightNow > window.innerWidth) this.destroy();
    }
},
...
function Enemy() {
    ...
    this.y = options.cellSize * 4 + getRandomInt(0,
        window.innerHeight - 10 - options.cellSize * 8);

    this.changeImg = function() {
        if (color.i === 0) {
            this.img.src = "images/alien" + getRandomInt(1, 4) + "/enemy5.png";
        } else {
            this.img.src = "images/alien" + getRandomInt(1, 4) +
                "/enemy" + Number(color.i - 1) +".png";
        }
    }

    this.returnToRespawn = function() {
        this.x = window.innerWidth + options.cellSize * 5;
        this.y = getRandomInt(0,
            window.innerHeight - 10 - options.cellSize * this.img.height * 3);
    ...
    this.update = function() {
        ...
        if (shot.isTrue &
            this.checkCollision([shot.coordConstY + options.cellSize,
...
function Alien(type) {
    ...
    this.draw = function() {
        ...
        if (color.i === 0) {
            ...
            this.img.src = "images/alien" + this.enemyType +
                "/enemy" + (this.colorStyle - 1) +".png";
        }
...
function keyboard() {
    if ((spaceShip.y > 0) & (!options.pause) &
        (keys.up)) spaceShip.y -= spaceShip.speed; // Up
    if ((spaceShip.y + options.cellSize * 3 + 20 < canvas.height) &
        (!options.pause) & (keys.down)) spaceShip.y += spaceShip.speed; // Down
    if ((spaceShip.x > 0) & (!options.pause) &
        (keys.left)) spaceShip.x -= spaceShip.speed; // Left
    if ((spaceShip.x + options.cellSize * 3 + 20 < canvas.width) &
        (!options.pause) & (keys.right)) spaceShip.x += spaceShip.speed; // Right
